...
class Graph():
    ...
    def __init__(self,
        ...
        self.adj_matrix_edges = {0: (1, 0), 1: (1, 2), 2: (3, 2), 3: (14, 8), 4: (14, 15),
                                 5: (16, 15), 6: (4, 0), 7: (4, 5), 8: (6, 5), 9: (11, 8),
                                 10: (11, 12), 11: (13, 12), 12: (7, 0), 13: (7, 8), 14: (9, 8),
                                 15: (9, 10)}

    def revert(self):
        # 初始化邻接矩阵，矩阵大小为字典的键的数量
        num_keys = len(self.adj_matrix_edges)
        adj_matrix = np.zeros((num_keys, num_keys), dtype=np.float32)

        # 根据字典键形成图
        for i in self.adj_matrix_edges:
            for j in self.adj_matrix_edges:
                if i != j and set(self.adj_matrix_edges[i]) & set(self.adj_matrix_edges[j]):  # 排除对角线元素
                    adj_matrix[i, j] = 1.0

        adj = normalize_digraph(adj_matrix)
        return adj
    ...
    def graph_link_between_frames(self, base):
        ...
        :return:
        """
        return [((front) + i * self.num_node_each, (back) + i * self.num_node_each) for i in range(self.seqlen) for
                (front, back) in base]  # 把每一帧的关节点都连接起来
    ...
    def compute_relative_distance_matrix(self):
        # 使用示例
        # 创建一个无向图
        G = nx.Graph()
        # 添加边，这里每条边的权重隐含为1
        G.add_edges_from([(0, 1), (2, 1), (3, 2), (4, 0), (5, 4), (6, 5),
                          (7, 0), (8, 7), (9, 8), (10, 9), (11, 8),
                          (12, 11), (13, 12), (14, 8), (15, 14), (16, 15)])
        graph = G
        """
        计算图的相对距离矩阵，假设每条边的长度为1。
        :param graph: networkx 图对象
        :return: 相对距离矩阵（numpy数组）
        """
        # 获取图中节点的数量
        ...
        # 将对角线上的元素设置为0，即节点到自身的距离
        np.fill_diagonal(distance_matrix, 0)

        # 遍历每个节点，执行BFS来计算最短路径
        for node in graph.nodes():
            # 使用BFS计算从该节点到其他所有节点的最短路径
    ...
    def Direction(self):
        # 输入的邻接关系
        edges = {
            0: (1, 0), 1: (1, 2), 2: (3, 2), 3: (14, 8), 4: (14, 15),
            5: (16, 15), 6: (4, 0), 7: (4, 5), 8: (6, 5), 9: (11, 8),
            10: (11, 12), 11: (13, 12), 12: (7, 0), 13: (7, 8), 14: (9, 8),
            15: (9, 10)
        }
        ...
        num_keys = len(edges)
        adj_matrix = np.zeros((num_keys, num_keys), dtype=np.float16)
        for i in edges:
            for j in edges:
                if i != j and set(edges[i]) & set(edges[j]):
                    adj_matrix[i, j] = 1.0
        adj_matrix = normalize_digraph(adj_matrix)
...
def get_hop_distance(num_node, edge, max_hop=1):  # 建立邻接矩阵,相邻则置0
    ...
    transfer_mat = [np.linalg.matrix_power(A, d) for d in
                    range(max_hop + 1)]  # GET [I,A]; matrix_power计算矩阵次方  0次方对角线全1，1次方不动
